...
public class AccelerationXInternalSensor extends GenericInternalSensor<ValueResult,Double, Dimensionless> {
private ValueResult value; //Object container of the value
	 ...
	 */
	public AccelerationXInternalSensor(byte sensorId,InternalMote owner) throws RemoteException
	{
...
//		this.value=new ValueResultImpl(2, "Decibel", measure, rangeTop, rangeDown, this.remoteSensor);
		this.sensorType = SensorType.ACCELEROMETERX;
		//System.out.println("Son un accelx sensor");
}
	...
	protected GenericRemoteSensor createRemote() {
		try {
			return new AccelerationXRemoteSensor(this);
		} catch (RemoteException e) {
			e.printStackTrace();
		}
		return remoteSensor;
