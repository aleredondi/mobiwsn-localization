...
/**
 * Class which is sued from the gateway to maintain the values reed from a noise sensor, in fact this object has inside
 ...
 * @version 1.0
 */
public class NoiseInternalSensor extends GenericInternalSensor<ValueResult,Double, Dimensionless> 
{
	 ...
	 */
	public NoiseInternalSensor(byte sensorId,InternalMote owner) throws RemoteException
	{
...
//		this.value=new ValueResultImpl(2, "Decibel", measure, rangeTop, rangeDown, this.remoteSensor);
		...
		//System.out.println("Son un noise sensor");
}
	...
	protected GenericRemoteSensor createRemote() {
		try {
			return new NoiseRemoteSensor(this);
		} catch (RemoteException e) {
			e.printStackTrace();
		}
		return remoteSensor;
